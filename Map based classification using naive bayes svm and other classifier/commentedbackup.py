...
def imgcrop(filename,verbose=False):
	...
	#print img.shape
	img_new = img[2922:5886,2226:5100] #reshape to desired row and column
	#print img_new
...
def image_to_matrix(filename, verbose=False):
	...
	print img_new.max()
	return img_new
imgarray=image_to_matrix("easyname/LT51470471999102XXX01_B1.TIF",False) #creating the flat array of first band image
...
imgarray=img_append("easyname/LT51470471999102XXX01_B2.TIF",imgarray) #appending band 2 to imgarray
imgarray=img_append("easyname/LT51470471999102XXX01_B3.TIF",imgarray) #appending band 3 to imgarray
imgarray=img_append("easyname/LT51470471999102XXX01_B4.TIF",imgarray) #appending band 4 to imgarray
imgarray=img_append("easyname/LT51470471999102XXX01_B5.TIF",imgarray) #appending band 5 to imgarray
imgarray=img_append("easyname/LT51470471999102XXX01_B6.TIF",imgarray) #appending band 6 to imgarray
...
clf=GaussianNB() #it most probably implements maximum likelihood classifier, yet to confirm this
clf.fit(X_train,y_train) # this fits training data to target set values, seems kinda redundant because we already did it for scikit while making target set 
...
y_pred=clf.predict(imgarray) #predicting target array for the whole resized image
y_pred1=clf.predict(X_test) #predicting target array for testing area only for making confusion matrix
print "y_pred"
print y_pred #check
print y_pred.shape #check
...
im=imgcrop("easyname/LT51470471999102XXX01_B1.TIF")

#below two steps might be unneseccary, check later
s=y_pred.shape[0]
y=y_pred.reshape(s,1)
y=y_pred.reshape(im.shape[0],im.shape[1])
print y #check
